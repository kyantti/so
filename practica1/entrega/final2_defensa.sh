...
while true; do
    ...
    1)
        ...
        while IFS="|" read -r email_id email_content spam_or_ham blank; do
            ...
            while read -r expression; do
                ...
                count=$(echo "$cleaned_email_content" | grep -wo "$cleaned_expression" | wc -l | tr -d '[:space:]')
    ...
    2)
        ...
        if [ "$freq_matrix_built" -eq 1 ]; then
            ...
            #----------------------MODIFICACION DEFENSA-------------------
            i=3
            percentage=0
            percentage_valid=0
            while [ "$i" -gt 0 ] && [ "$return_to_menu" -eq 0 ] && [ "$percentage_valid" -eq 0 ]; do
                read -rp "Introduzca el porcentaje para ser considerado SPAM: " percentage
            ...
            echo "Porcentaje seleccionado: $percentage"
            ...
            for ((i = 0; i < tfidf_matrix_rows; i++)); do
                ...
                #echo "TF-IDF Medio: $average_tf_idf"
                 if (($(echo "$average_tf_idf > percentage" | bc -l))); then
